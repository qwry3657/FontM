...
import com.sajjad.fontmanager.installer.MediaPlayerIconInstaller;
import com.sajjad.fontmanager.utils.AppUtils;
...
public class MediaPlayer extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mp_accent.setChecked(PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPA.overlay"));

        mp_accent.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                mp_system.setChecked(false);
                mp_pitch_black.setChecked(false);
                OverlayUtils.disableOverlay("Font_ManagerComponentMPS.overlay");
                OverlayUtils.disableOverlay("Font_ManagerComponentMPB.overlay");
                OverlayUtils.enableOverlay("Font_ManagerComponentMPA.overlay");
            } else {
                OverlayUtils.disableOverlay("Font_ManagerComponentMPA.overlay");
            }
            refreshPreview();
        });

        mp_system.setChecked(PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPS.overlay"));
        ...
        mp_system.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                ...
                OverlayUtils.disableOverlay("Font_ManagerComponentMPA.overlay");
                OverlayUtils.disableOverlay("Font_ManagerComponentMPB.overlay");
                OverlayUtils.enableOverlay("Font_ManagerComponentMPS.overlay");
            } else {
                OverlayUtils.disableOverlay("Font_ManagerComponentMPS.overlay");
            }
            refreshPreview();
        ...
        mp_pitch_black.setChecked(PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPB.overlay"));
        ...
        mp_pitch_black.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                ...
                OverlayUtils.disableOverlay("Font_ManagerComponentMPA.overlay");
                OverlayUtils.disableOverlay("Font_ManagerComponentMPS.overlay");
                OverlayUtils.enableOverlay("Font_ManagerComponentMPB.overlay");
            } else {
                OverlayUtils.disableOverlay("Font_ManagerComponentMPB.overlay");
            }
            refreshPreview();
    ...
    private void refreshPreview() {
        ...
        if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPA.overlay")) {
            preview_accent.setVisibility(View.VISIBLE);
            preview_system.setVisibility(View.GONE);
            preview_black.setVisibility(View.GONE);
        } else if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPB.overlay")) {
    ...
    private void enableOnClickListener(int aurora_id, int gradicon_id, int plumpy_id, int idx) {
        ...
        aurora.setOnClickListener(v -> {
            if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "1.overlay"))) {
                Shell.cmd("cmd overlay disable --user current Font_ManagerComponentMPIP" + idx + "1.overlay").exec();
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "1.overlay"), false);
            } else {
                MediaPlayerIconInstaller.install_pack(idx, 1);
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "1.overlay"), true);
            }
            checkIfApplied(aurora, gradicon, plumpy, idx);
        });

        gradicon.setOnClickListener(v -> {
            if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "2.overlay"))) {
                Shell.cmd("cmd overlay disable --user current Font_ManagerComponentMPIP" + idx + "2.overlay").exec();
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "2.overlay"), false);
            } else {
                MediaPlayerIconInstaller.install_pack(idx, 2);
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "2.overlay"), true);
            }
            checkIfApplied(aurora, gradicon, plumpy, idx);
        });

        plumpy.setOnClickListener(v -> {
            if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "3.overlay"))) {
                Shell.cmd("cmd overlay disable --user current Font_ManagerComponentMPIP" + idx + "3.overlay").exec();
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "3.overlay"), false);
            } else {
                MediaPlayerIconInstaller.install_pack(idx, 3);
                PrefConfig.savePrefBool(Font_Manager.getAppContext(), ("Font_ManagerComponentMPIP" + idx + "3.overlay"), true);
            }
            checkIfApplied(aurora, gradicon, plumpy, idx);
        });
    }
    ...
    private void refreshButton(Button btn1, Button btn2, Button btn3, int m) {
        if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPIP" + m + "1.overlay")) {
        ...
        } else if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPIP" + m + "2.overlay")) {
            btn2.setBackgroundResource(R.drawable.button_red);
            btn1.setBackgroundResource(R.drawable.button);
            btn3.setBackgroundResource(R.drawable.button);
        } else if (PrefConfig.loadPrefBool(Font_Manager.getAppContext(), "Font_ManagerComponentMPIP" + m + "3.overlay")) {
